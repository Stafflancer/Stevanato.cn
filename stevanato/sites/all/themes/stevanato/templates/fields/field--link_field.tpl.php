...
foreach ($items as $delta => $item) {
  // $alias = str_replace($base_url . '/', '', $item['#element']['display_url']);
  // $path = drupal_lookup_path('source', $alias);

  $path = str_replace($base_url . '/', '', $item['#element']['display_url']);

  if (explode('/', $path)[0] == 'node') {
    $node = menu_get_object('node', 1, $path);
    if (!empty($node->type) && $node->type == 'popup') {
      $items[$delta]['#element']['attributes']['class'] = 'autodialog';
    }
  }
}
?>
