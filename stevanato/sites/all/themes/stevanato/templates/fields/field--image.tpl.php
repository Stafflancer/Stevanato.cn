...
foreach ($items as $delta => $item) {
  $items[$delta]['class'] = '';
  if ($items[$delta]['#item']['filemime'] == 'image/gif') {
    $items[$delta]['#image_style'] = '';
    $items[$delta]['class'] = 'gif';
  }
}
...
<div class="<?php print $classes; ?>"<?php print $attributes; ?>>
  ...
  <div class="field-items"<?php print $content_attributes; ?>>
    <?php foreach ($items as $delta => $item): ?>
      <div class="field-item <?php print $delta % 2 ? 'odd' : 'even'; ?><?php print empty($item['class']) ? '' : ' ' . $item['class']; ?>"<?php print $item_attributes[$delta]; ?>><?php print render($item); ?></div>
    <?php endforeach; ?>
  </div>
</div>
